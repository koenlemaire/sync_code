...
time_norm=sqrt(length_norm/9.81); % [s] time normalization constant
...
fig.Position=[0 0 20 10]+2;
%fig.PaperPosition=fig.Position;

subtmp=subplot(3,4,[5 9]);
yoff=.4;
...
% COM
%plot(com(450,1)-xpos(1,1),com(450,2)+yoff,'k+','linewidth',1); hold on % stick figure
%plot(com(450,1)-xpos(1,1),com(450,2)+yoff,'ko','linewidth',1); hold on % stick figure
...
plot(t-take_off-.015,grf(:,2)/force_norm,'-','color',[minColor alphaMod(3)]) ; hold on
plot(t-take_off+0.015,grf(:,2)/force_norm,'-','color',[plusColor alphaMod(1)]); hold on
text(-.16,2.75,'$+15 \, \mathrm{ms}$','fontsize',7,'color',[plusColor alphaMod(1)])
text(-.28,2.85,'$-15 \, \mathrm{ms}$','fontsize',7,'color',[minColor alphaMod(3)])
...
ptmp=patch([t(1)-take_off+[0.030 0.030] -.015 -.015],[-.49 3 3 -.49],[.7 .7 .7]);
ptmp.FaceAlpha=.2;
...
for iLag=1:length(delayVec)
    ...
    for i=1:nseg-1
        ...
        idx_tmp=inv_Pjo(:,i)>0.01;        
        inv_Wjo_pos(iLag,i)=trapz(t_new(idx_tmp),inv_Pjo(idx_tmp,i));
        inv_Wjo_neg(iLag,i)=trapz(t_new(~idx_tmp),inv_Pjo(~idx_tmp,i));
    end
    ...
    %grf_err_tot(iLag)=sqrt(
    angle_error_tot(iLag,:)=sqrt(mean((phiJo(idx_old,:)-inv_phiJo(:,:)).^2));
    Mjo_tmp=Mjo(idx_old,:);
    Pjo_tmp=Pjo(idx_old,:);
    
    Mjo_error(iLag)=sqrt(mean((Mjo(idx_old,2)-inv_Mjo(:,2)).^2));
    Mjo_error_tot(iLag,:)=sqrt(mean((Mjo_tmp(lag/dt:end-lag/dt,:)-inv_Mjo(lag/dt:end-lag/dt,:)).^2))/moment_norm; % [dimless]
    ...
    Pjo_error_tot(iLag,:)=sqrt(mean((Pjo_tmp(lag/dt:end-lag/dt,:)-inv_Pjo((lag/dt:end-lag/dt),:)).^2))/power_norm; % [dimless]
    
    Fres_mag_series=sqrt(sum(inv_Fres.^2,2));
    Mres_mag_series=abs(inv_Mres);
    Fres_mag(iLag)=sqrt(mean(Fres_mag_series(lag/dt:end-lag/dt).^2)); % [N] mean of Fres magnitude
    Mres_mag(iLag)=sqrt(mean(Mres_mag_series(lag/dt:end-lag/dt).^2));
    ...
    if  iLag==1 || iLag==length(delayVec) || delayVec(iLag) == 0
        ...
        for iJo=1:3
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % moment
            tmp=subplot(3,4,iJo+5);
            yyaxis left
            tmp.YColor=[0 0 0];
            if iLag == 1
                plot(t_old(lag/dt:end-lag/dt)-take_off,inv_Mjo(lag/dt:end-lag/dt,iJo+1)/moment_norm/nr_legs,'-','color',[plusColor alphaMod(1)]); hold on % inverse result
            elseif iLag == length(delayVec)
                plot(t_old(lag/dt:end-lag/dt)-take_off,inv_Mjo(lag/dt:end-lag/dt,iJo+1)/moment_norm/nr_legs,'-','color',[minColor alphaMod(3)]) % inverse result
            else
                plot(t-take_off,Mjo(:,iJo+1)/moment_norm/nr_legs,'-','color',[Ncolor alphaMod(2)]); hold on % ground truth
            end
            box off
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % power
            tmp=subplot(3,4,iJo+9);
            yyaxis left
            tmp.YColor=[0 0 0];
            if iLag == 1
                plot(t_old(lag/dt:end-lag/dt)-take_off,inv_Pjo(lag/dt:end-lag/dt,iJo)/power_norm/nr_legs,'-','color',[plusColor alphaMod(1)]); hold on % inverse result
                
            elseif iLag == length(delayVec)
                plot(t_old(lag/dt:end-lag/dt)-take_off,inv_Pjo(lag/dt:end-lag/dt,iJo)/power_norm/nr_legs,'-','color',[minColor alphaMod(3)]) % inverse result
                if iJo==1
                    annotation('textarrow',[0.405 0.36],[0.24 0.24],'String','$+ \Delta t \,$')
                end
            else
                plot(t-take_off,Pjo(:,iJo)/power_norm/nr_legs,'-','color',[Ncolor alphaMod(2)]); hold on % ground truth
            end
            %tmp=text(1,1,'$\Delta t=15 \mathrm{ms}$','interpreter','latex','units','centimeters');
            %tmp=text(1,2,'$\Delta t=-15 \mathrm{ms}$','interpreter','latex','units','centimeters');
            box off
    ...
    if  iLag==1 || iLag==length(delayVec) || delayVec(iLag) == 0
        ...
        tmp.YColor=[0 0 0];
        if iLag == 1
            plot(t_old(lag/dt:end-lag/dt)-take_off,Fres_mag_series(lag/dt:end-lag/dt)/force_norm,'-','color',[plusColor alphaMod(1)]); hold on % ground truth
        elseif iLag==length(delayVec)
            plot(t_old(lag/dt:end-lag/dt)-take_off,Fres_mag_series(lag/dt:end-lag/dt)/force_norm,'-','color',[minColor alphaMod(3)]) % ground truth
            %text(tmp,lblx,lbly,'A','units','normalized')
            title('Jumping model')
            annotation('textarrow',[0.175 0.14],[0.80 0.80],'String','$+ \Delta t \,$')
        else
            plot(t-take_off,zeros(size(t)),'-','color',[Ncolor alphaMod(2)]);hold on
        end
        ...
        tmp.YColor=[0 0 0];
        if iLag == 1
            plot(t_old(lag/dt:end-lag/dt)-take_off,Mres_mag_series(lag/dt:end-lag/dt)/moment_norm,'-','color',[plusColor alphaMod(1)]); hold on % inverse result
        elseif iLag == length(delayVec)
            plot(t_old(lag/dt:end-lag/dt)-take_off,Mres_mag_series(lag/dt:end-lag/dt)/moment_norm,'-','color',[minColor alphaMod(3)]) % inverse result
            %text(tmp,lblx,lbly,'B','units','normalized')
        else
            plot(t-take_off,zeros(size(t)),'-','color',[Ncolor alphaMod(2)]); hold on % ground truth
        end
        box off
        %tmp=text(1,1,'$\Delta t=15 \mathrm{ms}$','interpreter','latex','units','centimeters');
        %tmp=text(1,2,'$\Delta t=-15 \mathrm{ms}$','interpreter','latex','units','centimeters');
...
yyaxis left
tmp.YColor=[0 0 0];
plot(delayVec*1000,Mjo_error_tot(:,2:3)/nr_legs,'k');
hold on; plot(delayVec*1000,Mjo_error_tot(:,4)/nr_legs,'k:','linewidth',1.1);
legend('ankle','knee','hip','Location','north')
legend box off
title('Jumping model')
%xlabel('delay [ms]')
...
yyaxis left
tmp.YColor=[0 0 0];
plot(delayVec*1000,Pjo_error_tot(:,1:2)/nr_legs,'k'); hold on
plot(delayVec*1000,Pjo_error_tot(:,3)/nr_legs,'k:','linewidth',1.1); hold on
%xlabel('$\Delta t$ (ms)')
...
%yyaxis('left')
tmp.YColor=[0 0 0];
plot(delayVec*1000,inv_Wjo_pos(:,1)/moment_norm/nr_legs,'k-'); hold on % ground truth
plot(delayVec*1000,inv_Wjo_pos(:,2)/moment_norm/nr_legs,'k--'); hold on % ground truth
plot(delayVec*1000,inv_Wjo_pos(:,3)/moment_norm/nr_legs,'k:','linewidth',1.1) % ground truth

plot(delayVec*1000,inv_Wjo_neg(:,1)/moment_norm/nr_legs,'k-'); hold on % ground truth
plot(delayVec*1000,inv_Wjo_neg(:,2)/moment_norm/nr_legs,'k--'); hold on % ground truth
plot(delayVec*1000,inv_Wjo_neg(:,3)/moment_norm/nr_legs,'k:','linewidth',1.1) % ground truth
...
ymax=[-.06 .24];
plot([0 0],ymax,'color',[0 0 0 .5])
plot([delayVec(1) delayVec(end)]*1000,[0 0],'color',[0 0 0 .5])
ylim(ymax)
yyaxis right
ylabel('(J)')
ylim(ymax*moment_norm)
tmp.YColor=[0 0 0];
box off
%xlim([-lag lag])
...
%% dim less errors
dimless_Pjo_RMSE=Pjo_error_tot(17,:)/time_norm*1000/nr_legs
Pjo_RMSE=Pjo_error_tot(17,:)*power_norm/nr_legs

dimless_Mjo_RMSE=Mjo_error_tot(17,:)/time_norm*1000/nr_legs
Mjo_RMSE=Mjo_error_tot(17,:)*moment_norm/nr_legs

F_err_mag=Fres_mag(17)
dimless_F_err_mag=Fres_mag(17)/force_norm/time_norm*1000

M_err_mag=Mres_mag(17)
dimless_M_err_mag=Mres_mag(17)/moment_norm/time_norm*1000
